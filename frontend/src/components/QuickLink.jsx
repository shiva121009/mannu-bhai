...
const QuickLinks = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-10">
      {/* Heading */}
      ...
      <div className="bg-white rounded-xl shadow border">
        {/* Header with toggle */}
        <div
          className="flex items-center justify-between px-4 sm:px-6 py-4 cursor-pointer border-b"
        ...
        {/* Cities Grid */}
        {isOpen && (
          <div className="p-4 sm:p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">
            {cities.map((city, index) => (
              <button
                key={index}
                className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm rounded-md bg-gray-100 hover:bg-gray-200 transition"
              >
                {city}
              </button>
            ))}
